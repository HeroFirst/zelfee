<script>
    $(document).ready(function(){
        $('#modalAccept').on('show.bs.modal', function (event) {
            ...
            var modal = $(this);
            modal.find('.modal-body p').text('Подтвердить заказ #' + order_id + '?');
...
<div class="modal modal-warning fade" id="modalAccept" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            ...
            <div class="modal-footer">
                <form id="formAccept" method="post" action="/private/store/orders/accept">
                    <input type="hidden" name="order_id" value="">
                </form>
                <button type="button" class="btn btn-outline pull-left" data-dismiss="modal">Отменить</button>
                <button form="formAccept" type="submit" class="btn btn-outline">Подтвердить</button>
            </div>
        </div>
